...
class ProfFamiliesController extends Controller {
	...
	public function getAllProfFamiliesViewInactives() {
		...
		$urlPost = config('routes.admin.profFamilieReActivate');

		return view('admin.profFamilies.inactives', compact('profFamiliesInactives', 'zona', 'urlPost'));
	}
	...
	/**
	 * Método de edición de familias profesionales
	 * @param  Request $request La petición post
	 * @param  inte  $id        El id de la familia profesional
	 */
	...
	public function delete(Request $request, $id = null) {
		...
		if ($profFamilie->deleted_at == null) {
			...
			// No tiene porque tener ciclos, eso debe controlarse
			$cicles = Cycle::where('profFamilie_id', '=', $id)->get();

			// si tiene ciclos, los borramos y los borramos de la n:m donde están las asignaturas
			if ((isset($cicles)) && (!empty($cicles))) {

				$cyclesId = $this->idToArray($cicles);

				\DB::beginTransaction();

				$CyclesSubject = \DB::table('cycleSubjects')->whereIn('cycle_id', $cyclesId)->get();

				if ($CyclesSubject === false) {
					\DB::rolback();
					// Devolvemos un mensaje a la vista
					$message = 'No se ha podido borrar los ciclos asociados a la familia profesional, por favor intentelo más tarde';
					$status = 'fail';
					...
					$message = 'La familia profesional y los ciclos asociados a la familia profesional han sido borrados correctamente';
					$status = 'success';
				...
				// sino, solo borramos la familia profesional
				// Borramos el profFamily
				$profFamilie->deleted_at = date('YmdHms');
				$profFamilie->active = 0;
				$profFamilie->save();
	 ...
	 */
	protected function restoreDeniedProfFamilie($request) {
		// Array de las ofertas a validar
		$profFamilies = $request->toArray();

		foreach ($profFamilies['profFamilie'] as $id => $value) {

			// Comprobamos que el oferta esta borrado
			$denieProfFamilie = $this->getDeniedProfFamilie($value);

			// Si esta borrado lo restauramos
			if ($denieProfFamilie) {

				$denieProfFamilie->deleted_at = null;

				$denieProfFamilie->save();
