    ...
    {{-- Comprobamos si existen ofertas verificadas --}}
        @if(isset($workCenters))
            ...
            @foreach($workCenters as $key => $workCenter)
                <div class="{{(count($workCenters) % 2 == 0) ? 'col-md-12' : 'col-md-6' }} oferta extra-padding-bottom mask hoverable scroll z-depth-1" tabindex="{{ $workCenter->id }}">
                    <div class="col-md-12" data-id="{{(isset($workCenter->id)) ? $workCenter->id : 0 }}" id="{{(isset($workCenter->id)) ? $workCenter->id : 0 }}">
                        ...
                        <div class="row extra-padding-bottom">
                            <div class="col-sm-7" data-enterprise="{!! $workCenter->address !!}"><b>Empresa: </b><a href="{!! (isset($workCenter->web)) ? $workCenter->web : 'https://www.google.es/#q='.$workCenter->address !!}" target="_blank">{!! $workCenter->address !!} <i class="fa fa-link" aria-hidden="true"></i></a></div>
                            <div class="col-sm-5" data-city="{!! $workCenter->road !!}"><b>Lugar: </b><a href="https://www.google.es/maps/place/{!! $workCenter->road !!}" target="_blank" data-lugar="{!! $workCenter->road !!}">{!! $workCenter->road !!} <i class="fa fa-link" aria-hidden="true"></i></a></div>
                        </div>
                        <div class="row extra-padding-bottom">
